 ...
 * @param color 颜色
 */
void OLED_PrintNum(uint8_t x, uint8_t y, int num, const Font *font, OLED_ColorMode color)
{
    char str[12]; // 假设整数不超过32位（最多10位 + 1符号 + 1结束符）
 ...
 * @param color 颜色
 */
void OLED_PrintFloatNum(uint8_t x, uint8_t y, float num, const Font *font, OLED_ColorMode color)
{
    char str[12]; // 假设整数不超过32位（最多10位 + 1符号 + 1结束符）
    sprintf(str, "%.2f", num); // 将整数转换为字符串
    OLED_PrintString(x, y, str,font, color); 
}
